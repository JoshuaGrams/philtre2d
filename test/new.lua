...
return {
	"New",
	function()
		local isSuccess, result = pcall(new.custom, "testType", loader, "one", "two", "three")
		T.ok(isSuccess, "new.custom works without errors.")
		local asset = result
		T.is(type(asset), "table", "Asset is the correct type.")
		T.has(asset, {one=true,two=true,three=true}, "Asset is what we tried to create.")
		local asset2 = new.custom("testType", loader, "one", "two", "three")
		T.is(asset2, asset, "Second call to new.custom with the same arguments gives the same asset.")
		T.is(loadCalls, 1, "Second call to new.custom with the same args doesn't call loader again.")
		-- Refcount & unloading.
		...
		local asset = new.custom("testType", loader, "one", "two", "three")
		...
		-- Manual unloading.
		local asset3 = new.custom("testType", loader, "one", "two", "three")
		local asset4 = new.custom("testType", loader, "one", "two", "three")
		local asset5 = new.custom("testType", loader, "one", "two", "three")
