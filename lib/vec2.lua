-- Cirno's Perfect Math Library - vec2 module - with tiny additions by Ross

-- Ross Additions:
--		vec2.lerpdt(a, b, s, dt) -- lerp with dt
-- 		vec2.angle_between -- removed fallback to vec3?
--		vec2.new(x, y) -- if x is a vec2, return a copy of it

-- (Note: __mul is per-value multiplication)
...
-- @field unit_scale Base scale vector
vec2.unit_x = new(1, 0)
vec2.unit_y = new(0, 1)
vec2.zero   = new(0, 0)
...
-- @tparam vec2 a Left hand operand
-- @tparam vec2 b Right hand operand
-- @tparam number s Step value per second
-- @tparam number dt Delta time
-- @treturn vec2 out
function vec2.lerpdt(a, b, s, dt)
